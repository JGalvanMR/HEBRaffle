...
public sealed class ExportService : IExportService
{
    ...
    // ── Lazy: el directorio se crea en el primer uso, no en el constructor.
    //    Evita fallos en el sandbox de iOS durante la fase de inicialización del DI.
    //    En iOS: ~/Documents/HEBRaffle → visible en app "Archivos" (UIFileSharingEnabled=true)
    //    En Android: ~/Documents/HEBRaffle → directorio privado de la app
    private string? _exportFolder;
    private string ExportFolder
    {
        get
        {
            if (_exportFolder is not null) return _exportFolder;
    ...
    public async Task<string> ExportParticipantsAsync(CancellationToken ct = default)
    {
        ...
        await Task.Run(() =>
        {
            ...
            SetupHeader(ws, ["#", "First Name", "Last Name", "Full Name",
                              "Store", "Years in Company", "Registration Date", "Winner"]);
            ...
            for (int i = 0; i < participants.Count; i++)
            {
                ...
                if (p.IsWinner)
                    ws.Range(row, 1, row, 8).Style.Fill.SetBackgroundColor(XLColor.LightGreen);
            }
    ...
    public async Task<string> ExportWinnersAsync(CancellationToken ct = default)
    {
        ...
        await Task.Run(() =>
        {
            ...
            for (int i = 0; i < winners.Count; i++)
            {
                ...
                if (i % 2 == 1)
                    ws.Range(row, 1, row, 7).Style.Fill.SetBackgroundColor(XLColor.FromHtml("#FFF5F5"));
            }
